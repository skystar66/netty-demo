...
//    private static final Map<Integer, MessageQueue<ServerInfoVO>> fromRPCMsgQueueMap = new HashMap<>();
    private static final Map<Integer, MessageQueue<ServerInfoVO>> retryConnectQueueMap = new HashMap<>();
...
//        }
        for (int i = 0; i < Constants.retryQueueCount; i++) {//cpu*2个队列对应cpu*2个消费线程
            retryConnectQueueMap.put(i, new DefaultMQ<>());
        }
    }

    /**得到重试队列*/
    public static MessageQueue<ServerInfoVO> getRetryConnectQueue(){
        return retryConnectQueueMap.get(RandomUtils.nextInt(0, Constants.retryQueueCount));
    }
    /**得到重试队列*/
    public static MessageQueue<ServerInfoVO> getRetryConnectQueueByIndex(int index){
